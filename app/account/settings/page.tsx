...
import { getAuthUser, logoutUser, loginUser, UserSession } from "@/lib/authHelper";
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      setSettings(s => ({ ...s, firstName: parts[0] || "", lastName: parts.slice(1).join(" ") || "", email: user.email }));
    }
    const handleAuth = () => {
      const u = getAuthUser();
      if (!u) {
        ...
        setSettings(s => ({ ...s, firstName: parts[0] || "", lastName: parts.slice(1).join(" ") || "", email: u.email }));
      }
    };
      ...
      <main className="max-w-container-max mx-auto px-4 sm:px-6 md:px-margin-desktop py-8 sm:py-12">
        <div className="flex flex-col lg:flex-row gap-8 sm:gap-12 items-start">
          ...
          <aside className="w-full lg:w-64 flex-shrink-0">
            <h2 className="font-display-xl text-3xl sm:text-4xl uppercase mb-2">My Account</h2>
            {authUser && (
            ...
            <nav className="flex flex-col gap-3">
              ...
              {/* Active */}
              <Link className="flex items-center justify-between py-3 border-b-2 border-milano-red" href="/account/settings">
          ...
          <section className="flex-1 w-full">
            {!authUser ? (
              <div className="border-4 border-on-surface p-8 sm:p-12 text-center bg-surface shadow-[8px_8px_0px_0px_#a90e02]">
                ...
                <p className="font-body-md text-sm opacity-70 mb-6 uppercase tracking-wider">Please sign in to view and update your profile & security settings.</p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  ...
                  <button
                    onClick={() => loginUser("vedant@thedrop.com", "password")}
                    className="px-8 py-3.5 border-2 border-on-surface font-label-bold text-xs uppercase tracking-widest hover:bg-on-surface hover:text-lemon-chiffon transition-colors cursor-pointer"
                  >
                    DEMO VIP SIGN IN
                  </button>
                </div>
            ...
            <div className="flex border-b-2 border-on-surface mb-8">
              {(["profile", "notifications", "preferences"] as const).map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-3 font-label-bold text-xs uppercase tracking-widest transition-colors cursor-pointer border-r border-on-surface/20 ${activeTab === tab ? "bg-on-surface text-lemon-chiffon" : "bg-transparent text-on-surface hover:bg-on-surface/10"}`}>
                  {tab}
                </button>
              ))}
            </div>
            ...
            <form onSubmit={handleSave}>
              {activeTab === "profile" && (
                <div className="space-y-8">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div><label className={labelClass}>First Name</label><input className={inputClass} value={settings.firstName} onChange={(e) => setSettings({ ...settings, firstName: e.target.value })} /></div>
                    <div><label className={labelClass}>Last Name</label><input className={inputClass} value={settings.lastName} onChange={(e) => setSettings({ ...settings, lastName: e.target.value })} /></div>
                    <div><label className={labelClass}>Email Address</label><input type="email" className={inputClass} value={settings.email} onChange={(e) => setSettings({ ...settings, email: e.target.value })} /></div>
                    <div><label className={labelClass}>Phone Number</label><input type="tel" className={inputClass} value={settings.phone} onChange={(e) => setSettings({ ...settings, phone: e.target.value })} /></div>
                  </div>
                  <div className="pt-2 border-t-2 border-on-surface/10">
                    ...
                    <button type="button" onClick={() => setShowPasswordModal(true)} className="flex items-center gap-2 px-5 py-2.5 border-2 border-on-surface font-label-bold text-xs uppercase tracking-wider hover:bg-on-surface hover:text-lemon-chiffon transition-colors cursor-pointer">
                ...
                <div className="space-y-6">
                  {([
                    ...
                    <div key={key} className="flex items-start justify-between gap-4 py-4 border-b border-on-surface/10">
                      ...
                      <button type="button" onClick={() => setSettings({ ...settings, [key]: !settings[key as keyof typeof settings] })} className={`relative w-12 h-6 rounded-full border-2 transition-colors flex-shrink-0 cursor-pointer ${settings[key as keyof typeof settings] ? "bg-milano-red border-milano-red" : "bg-on-surface/10 border-on-surface/30"}`}>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      ...
                      <select className={`${inputClass} cursor-pointer`} value={settings.currency} onChange={(e) => setSettings({ ...settings, currency: e.target.value })}>
                        {["INR", "USD", "EUR", "GBP", "JPY", "AED"].map((c) => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className={labelClass}>Language</label>
                      <select className={`${inputClass} cursor-pointer`} value={settings.language} onChange={(e) => setSettings({ ...settings, language: e.target.value })}>
                        {["English", "Hindi", "Japanese", "French", "German", "Arabic"].map((l) => <option key={l} value={l}>{l}</option>)}
                      </select>
                    </div>
                  </div>
              ...
              <div className="mt-8 pt-6 border-t-2 border-on-surface/10">
                <button type="submit" className="px-10 py-3.5 bg-milano-red text-lemon-chiffon font-headline-md text-sm uppercase tracking-widest hover:bg-on-surface transition-colors border-2 border-on-surface cursor-pointer">
                  Save Changes
                </button>
              </div>
            </form>
        ...
        <div className="fixed inset-0 z-50 bg-on-surface/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-lemon-chiffon border-4 border-on-surface w-full max-w-md shadow-[8px_8px_0px_0px_#a90e02]">
            ...
            <form onSubmit={handlePasswordSave} className="p-6 space-y-5">
              <div><label className={labelClass}>Current Password</label><input type="password" required className={inputClass} placeholder="••••••••" value={passwords.current} onChange={(e) => setPasswords({ ...passwords, current: e.target.value })} /></div>
              <div><label className={labelClass}>New Password</label><input type="password" required className={inputClass} placeholder="Min 8 characters" value={passwords.next} onChange={(e) => setPasswords({ ...passwords, next: e.target.value })} /></div>
              <div><label className={labelClass}>Confirm New Password</label><input type="password" required className={inputClass} placeholder="Repeat new password" value={passwords.confirm} onChange={(e) => setPasswords({ ...passwords, confirm: e.target.value })} /></div>
              {pwError && <p className="text-milano-red font-label-bold text-xs uppercase">{pwError}</p>}
              <div className="flex gap-3 pt-2">
                <button type="submit" className="flex-1 py-3 bg-milano-red text-lemon-chiffon font-headline-md uppercase tracking-widest hover:bg-on-surface transition-colors cursor-pointer border-2 border-on-surface">Update Password</button>
                <button type="button" onClick={() => setShowPasswordModal(false)} className="px-6 py-3 border-2 border-on-surface font-label-bold text-xs uppercase hover:bg-on-surface hover:text-lemon-chiffon transition-colors cursor-pointer">Cancel</button>
              </div>
            </form>
          </div>
        </div>
