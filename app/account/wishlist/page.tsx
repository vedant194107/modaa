...
export default function WishlistPage() {
  ...
  const handleRemove = (item: WishlistItem) => toggleWishlistItem(item);
  ...
  const handleShareWishlist = () => {
    const shareableUrl = `${window.location.origin}/wishlist?shared=1&ids=${encodeURIComponent(JSON.stringify(wishlist.map(i => i.id)))}`;
        ...
        <div className="flex flex-col lg:flex-row gap-8 sm:gap-12 items-start">
          ...
          <aside className="w-full lg:w-64 flex-shrink-0">
            ...
            <nav className="flex flex-col gap-3">
              ...
              {/* Active */}
              <Link className="flex items-center justify-between py-3 border-b-2 border-milano-red" href="/account/wishlist">
          ...
          <section className="flex-1 w-full">
            {!authUser ? (
              <div className="border-4 border-on-surface p-8 sm:p-12 text-center bg-surface shadow-[8px_8px_0px_0px_#a90e02]">
                ...
                <p className="font-body-md text-sm opacity-70 mb-6 uppercase tracking-wider">Please sign in to view your saved wishlist items.</p>
              ...
              <>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 mb-6 border-b-2 border-on-surface pb-4">
                  <div>
                  ...
                  {wishlist.length > 0 && (
                    <button
                      ...
                      className="font-label-bold text-xs uppercase bg-milano-red text-lemon-chiffon px-4 py-2 hover:bg-on-surface transition-colors flex items-center gap-1.5 border-2 border-on-surface cursor-pointer font-bold"
                    >
                      <span className="material-symbols-outlined text-base">share</span>
                      {copiedLink ? "✓ WISHLIST LINK COPIED!" : "SHARE WISHLIST WITH FRIENDS"}
                    </button>
                  )}
                </div>
            ...
            {wishlist.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 gap-6 text-center">
                ...
                <div>
                  ...
                  <p className="font-label-bold text-sm text-on-surface/60 uppercase tracking-wider">Browse the collection and hit ♥ to save pieces</p>
                </div>
                <Link href="/products" className="mt-4 px-8 py-4 bg-on-surface text-lemon-chiffon font-headline-md text-sm uppercase tracking-widest hover:bg-milano-red transition-colors border-2 border-on-surface">
                  Browse The Drop
                </Link>
              </div>
            ) : (
              <>
                  ...
                  {wishlist.map((item) => (
                    <div key={item.id} className="group relative flex flex-col border-2 border-on-surface bg-surface overflow-hidden">
                      <div className="relative aspect-[3/4] overflow-hidden bg-white">
                        ...
                        <button onClick={() => handleRemove(item)} className="absolute top-3 right-3 w-9 h-9 bg-milano-red text-lemon-chiffon flex items-center justify-center border-2 border-on-surface hover:bg-on-surface transition-colors z-10" title="Remove from Wishlist">
                        ...
                        <div className="absolute bottom-0 left-0 w-full bg-on-surface/90 backdrop-blur-sm translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-10 p-2 sm:p-3">
                          <button onClick={() => handleMoveToBag(item)} className="w-full bg-milano-red text-lemon-chiffon font-headline-md py-2 text-xs sm:text-sm uppercase tracking-wider hover:bg-white hover:text-milano-red transition-colors cursor-pointer">Move to Bag</button>
                        </div>
                      </div>
                      <div className="p-3 border-t-2 border-on-surface flex flex-col gap-1">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <button onClick={() => handleMoveToBag(item)} className="text-[9px] sm:text-[10px] font-label-bold uppercase tracking-wider text-on-surface/60 hover:text-milano-red transition-colors border border-on-surface/30 px-2 py-1 hover:border-milano-red">
                            Add to Bag
                          </button>
                        </div>
                      </div>
                ...
                <div className="mt-12 flex flex-col sm:flex-row items-center justify-between gap-4 border-t-2 border-on-surface pt-8">
                  <Link href="/products" className="font-label-bold text-sm uppercase tracking-widest text-on-surface/60 hover:text-milano-red transition-colors flex items-center gap-2">
                    <span className="material-symbols-outlined text-lg">arrow_back</span>
                    Continue Shopping
                  </Link>
                  <button
                    onClick={() => {
                      wishlist.forEach((item) => addItemToCart({ id: item.id, title: item.title, price: typeof item.price === "string" ? parseFloat(item.price.replace(/[^0-9.]/g, "")) : item.price, image: item.image }));
